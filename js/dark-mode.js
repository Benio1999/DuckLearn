(function(){
  ...
  } //abre uma função pra aplicar dark tema
  ...
  } //cria uma função pra atualizar o icone do botão para alternar o tema

  function toggleTheme(){
    const isDark = document.documentElement.classList.toggle('dark-theme');
    localStorage.setItem(KEY, isDark ? '1' : '0');
    updateToggleIcon(isDark);
  }//cria a função pra salvar a alteração do tema no local storage
  ...
  function createToggle(){
    ...
    document.head.appendChild(style); //adiciona o estilo do botão no head do documento
  ...
  }//cria o botão para alterar o tema

  function init(){
    const stored = localStorage.getItem(KEY);
    const isDark = stored === '1';
    applyTheme(isDark);
    // cria uma validação para garantir que o DOM esteja carregando para adicionar o botão
    if(document.readyState === 'loading'){
      window.addEventListener('DOMContentLoaded', createToggle);
    } else {
      createToggle();
    }
  }//função para inciar o script

  window.toggleDarkTheme = toggleTheme;
  window.applyDarkTheme = applyTheme;
  //aplica o tema claro ou escuro na pagina
